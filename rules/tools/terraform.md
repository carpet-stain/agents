...
> template's Terraform overlay demonstrably delivers this content — the interregnum rule
> (agents#90 Phase 3). Only then does PST become canonical, not before — and the `paths:` gate
> above dissolves at the same point, since a Terraform repo knows it's a Terraform repo. Removal
